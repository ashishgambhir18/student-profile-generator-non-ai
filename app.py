...
if st.session_state.stage == "upload":
    ...
    # A maximum of 5 statements can be selected across ALL categories.
    selections = {}
    saved_sel = ws.get("selections", {})
    saved_total = sum(len(v) for v in saved_sel.values()) if isinstance(saved_sel, dict) else 0
    current_total = 0

    st.caption(f"Select up to **5 statements** for this profile. Selected: **{saved_total}/5**")
    ...
    for cat, statements in bank.items():
        ...
        with st.expander(f"{cat} ({len(statements)} statements)", expanded=False):
            for idx, label in enumerate(labels):
                ...
                disabled = (current_total >= 5) and not is_checked
    ...
    # Enforce the five-statement cap even if stale browser/session state contains
    # more than five older selections. Keep the first five in category order.
    if current_total > 5:
        remaining = 5
        trimmed = {}
    ...
    st.caption(f"**{current_total}/5 statements selected**")
    if current_total >= 5:
        st.info("Maximum of 5 statements selected. Uncheck a statement to choose a different one.")
