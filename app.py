...
def _local_fit_profile(text, min_len=366, max_len=390):
    ...
    removals = [
        ...
        r"\bcurrently\b", r"\balso\b", r"\bonly\b", r"\bjust\b",
    ]
    out = s
    for pat in removals:
...
{target}"""
        try:
            ...
            # Models occasionally miss the boundary by only a few characters.
            # Correct small overruns locally instead of making the teacher wait
            # through several more API calls.
            if 390 < n <= 410 and candidate.startswith(first_name):
