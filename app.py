...
TEMPLATE_PATH = os.path.join(os.path.dirname(__file__), "Class_List_Template.xlsx")
SCHOOL_LOGO_URL = "https://pbs.twimg.com/profile_images/454645958068797441/TBZZx-IG.jpeg"

AI_MODEL = "gpt-5.6-luna"

# -----------------------------------------------------------------------------
# Browser-local persistence (Streamlit Components V2)
# -----------------------------------------------------------------------------
# Uses the browser's localStorage so a teacher can close the app and resume later.
# Components V2 avoids the duplicate-registration problem that affected the
# earlier V1 custom component.
STORAGE_KEY = "student_profile_generator_v15"

_BROWSER_STORAGE = st.components.v2.component(
    "student_profile_browser_storage_v16",
    html="<div></div>",
    js=r"""
export default function ({ data, setStateValue }) {
    ...
    if (action === "load") {
        ...
        try {
            window.localStorage.setItem(storageKey, data?.payload || "");
            setStateValue("ack", requestId || "saved");
        } catch (e) {
            setStateValue("error", String(e));
        }
    } else if (action === "clear") {
        window.localStorage.removeItem(storageKey);
        setStateValue("stored", "");
        setStateValue("ack", requestId || "cleared");
    }
}
""",
)
...
def storage_load():
    result = _BROWSER_STORAGE(
        data={"action": "load", "storageKey": STORAGE_KEY},
        default={"stored": ""},
        on_stored_change=lambda: None,
        key="browser_storage_load_v16",
    )
    return getattr(result, "stored", "") or ""


def storage_save(payload):
    request_id = str(st.session_state.get("storage_request_id", 0) + 1)
    st.session_state.storage_request_id = int(request_id)
...
def _local_fit_profile(text, min_len=366, max_len=390):
    """Make a very small, grammar-safe length correction when the AI misses narrowly."""
    ...
    # First remove harmless filler phrases, longest first.
    removals = [
        # Prefer removing optional intensifiers/fillers before touching meaning.
        r"\bvery\s+good\b", r"\bvery\s+well\b", r"\bquite\s+well\b",
        r"\bin\s+general\b", r"\bto\s+some\s+extent\b", r"\bwhere\s+appropriate\b",
        r"\baptly\b", r"\beffectively\b", r"\bproperly\b", r"\bthoroughly\b",
        r"\baccurately\b", r"\bconfidently\b", r"\bcarefully\b", r"\bregularly\b",
        r"\bconsistently\b", r"\bgenerally\b", r"\bparticularly\b", r"\bappropriately\b",
        r"\btruly\b", r"\breally\b", r"\bproactively\b", r"\benthusiastically\b",
        r"\bcurrently\b", r"\balso\b", r"\bonly\b", r"\bjust\b", r"\bnow\b",
        r"\bgreater\b", r"\bappropriate\b", r"\bimportant\b", r"\bcontinued\b",
        r"\bneeded\b", r"\bconsistent\b", r"\bstrong\b", r"\bgood\b", r"\bclear\b",
    ]
    out = s
    for pat in removals:
        while len(out) > max_len and re.search(pat, out, flags=re.I):
            out = re.sub(pat, "", out, count=1, flags=re.I)
            out = re.sub(r"\s{2,}", " ", out).strip()
    # Remove a leading conjunction if a deletion left one.
    out = re.sub(r"\s+([,.])", r"\1", out)
    if len(out) <= max_len and len(out) >= min_len:
        return out
    return None


def _extract_response_text(response):
    """Extract text robustly across OpenAI Responses SDK response shapes/versions."""
    # Preferred convenience property in current SDKs.
    ...
    def collect(value):
        ...
        if isinstance(value, dict):
            # Text can be represented as {"value": "..."} in some SDK shapes.
            if isinstance(value.get("text"), str) and value["text"].strip():
                return [value["text"]]
            if isinstance(value.get("value"), str) and value["value"].strip():
                return [value["value"]]
            chunks = []
            # Walk only response-content fields to avoid accidentally collecting
            # metadata, annotations or unrelated strings.
            for key in ("output", "content", "text", "value"):
                if key in value:
        ...
        # Typed SDK objects: inspect their content/output fields, and then text/value.
        for attr in ("output", "content", "text", "value"):
            try:
    ...
    chunks = collect(getattr(response, "output", None))
...
def ai_edit_profile(profile, subject, name, gender, mode="adjust", max_attempts=5):
    ...
    for attempt in range(max_attempts):
        target = (
            "Aim for 375–380 characters; keep a safety margin below 390."
            if attempt == 0
            else "MUST be 366–390 characters inclusive. This is a strict editing task: return about 375 characters (prefer 370–380), and do not return a draft outside 366–390. If the latest draft is already close to the range, make only the smallest necessary wording changes to bring it inside the range."
        )
...
{target}"""
        try:
            ...
            # Models occasionally miss the boundary by a small amount. First
            # try a conservative local repair; this avoids another API call for
            # harmless filler words.
            if 390 < n <= 430 and candidate.startswith(first_name):
                fitted = _local_fit_profile(candidate)
                if fitted and 366 <= len(fitted) <= 390:
                    return fitted, len(fitted), "ai"

            # If the response is outside the range, make the next attempt work
            # directly on the latest candidate instead of regenerating from the
            # original profile. This handles both over-long AND under-length
            # responses (for example 401 or 302 characters) much more reliably.
            if candidate and candidate.startswith(first_name):
                anonymized = re.sub(
                ...
                if n > 390:
                    task = "SHORTEN THE LATEST DRAFT. Do not regenerate from the original. Preserve its meaning, important observations and grammar. Remove redundancy, repeated ideas and non-essential modifiers. The final response must be 366–390 characters; target 370–380. Do not return 391 or more characters."
                elif n < 366:
                    task = "EXPAND THE LATEST DRAFT. Do not regenerate from the original. Preserve every important teacher observation already present. Add only concise elaboration of the same observations or a closely related recommendation supported by the existing content. The final response must be 366–390 characters; target 370–380. Do not invent facts, achievements, weaknesses, topics or behaviour."
...
def restore_state(raw):
    try:
        data = json.loads(raw)
        if not isinstance(data, dict) or data.get("version") not in (15, 16):
            return False
...
def sync_current_workspace():
    ...
    for cat, statements in bank.items():
        selected_indices = []
        for idx, _statement in enumerate(statements):
            key = f"check_{subject}_{i}_{cat}_{idx}"
            if bool(st.session_state.get(key, False)):
                selected_indices.append(idx)
        sel[cat] = selected_indices

    # Keep persistence consistent with the five-statement limit.
    total = sum(len(v) for v in sel.values())
    if total > 5:
        remaining = 5
        trimmed = {}
        for cat, indices in sel.items():
...
with header_left:
    ...
    st.caption("The Shri Ram School, Aravali • Built-in statement banks • AI is used only when you explicitly click Adjust or Refine • Every final profile: 366–390 characters")
with header_right:
    st.image(SCHOOL_LOGO_URL, width=78)

st.sidebar.header("Children")

# Make the class-list template available to every teacher directly inside the app.
# The file is bundled with the application, so teachers do not need a separate link.
try:
    ...
    st.sidebar.download_button(
        ...
        data=template_bytes,
        file_name="Class_List_Template.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        help="Download the Excel template. Fill in Name and Gender, then upload it below.",
    )
except Exception:
    st.sidebar.caption("Class-list template is temporarily unavailable.")

st.sidebar.markdown("[Visit The Shri Ram School website](https://tsrs.org/)")
st.sidebar.divider()
...
if st.session_state.stage == "upload":
    ...
    st.info("New here? Use **📥 Download Class List Template** in the left sidebar, fill in the student names and genders, and upload the completed file here.")
    ...
    with c2:
        if st.button("Start profiles →", type="primary"):
            ...
            st.session_state.profile_draft = ""
            st.session_state.ai_profile = ""
            # Keep existing workspaces when switching subjects so previously
            # entered work is preserved. Create missing workspaces only.
            for idx in range(len(st.session_state.students)):
                st.session_state.workspaces.setdefault(idx, blank_workspace())
            st.session_state.stage = "profile"
            request_save()
            st.rerun()
    ...
    with nav1:
        if st.button("⬅️ Change Subject", use_container_width=True):
            # Return to subject selection without clearing the class list or
            # any saved/draft workspaces.
            request_save()
            st.session_state.stage = "subject"
            st.rerun()
    st.markdown(f"### {name}")
    ...
    # Statement selection uses individual checkboxes rather than multiselects.
    # A maximum of 7 statements can be selected across ALL categories.
    selections = {}
    saved_sel = ws.get("selections", {})
    saved_total = sum(len(v) for v in saved_sel.values()) if isinstance(saved_sel, dict) else 0
    current_total = 0

    st.caption(f"Select up to **7 statements** for this profile. Selected: **{saved_total}/7**")

    for cat, statements in bank.items():
        labels = [prepare(statement, name, gender, first_sentence=False) for statement in statements]
        saved_indices = set(saved_sel.get(cat, [])) if isinstance(saved_sel, dict) else set()
        cat_selected = []

        with st.expander(f"{cat} ({len(statements)} statements)", expanded=False):
            for idx, label in enumerate(labels):
                key = f"check_{subject}_{i}_{cat}_{idx}"
                if key not in st.session_state:
                    st.session_state[key] = idx in saved_indices
                is_checked = bool(st.session_state.get(key, False))
                # Allow already-selected statements to be unchecked even when
                # the five-statement limit has been reached.
                disabled = (current_total >= 7) and not is_checked
                checked = st.checkbox(label, key=key, disabled=disabled)
                if checked:
                    cat_selected.append(idx)
                    current_total += 1

        selections[cat] = cat_selected

    # Enforce the seven-statement cap even if stale browser/session state contains
    # more than seven older selections. Keep the first seven in category order.
    if current_total > 7:
        remaining = 7
        trimmed = {}
    ...
    st.caption(f"**{current_total}/7 statements selected**")
    if current_total >= 7:
        st.info("Maximum of 7 statements selected. Uncheck a statement to choose a different one.")
