...
{target}"""
        try:
            ...
            # If the response is still slightly too long, make the next attempt
            # explicitly shorten the latest candidate rather than regenerating
            # from scratch. This is much more reliable for 391–410 character
            # responses.
            if n > 390 and n <= 430:
                # Feed the latest draft back to the model, but anonymize the
                # student's real first name again. Otherwise the retry prompt
                # requires the word STUDENT while the input already contains
                # the real name, which can make the model less reliable.
                anonymized = re.sub(
