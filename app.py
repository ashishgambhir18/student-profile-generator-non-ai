...
def prepare(statement, name, gender, first_sentence=True):
    ...
    # Normalise source gender variants first.
    s = s.replace("his /her", p["poss"]).replace("his/her", p["poss"])
    s = s.replace("His /her", p["Poss"] if "Poss" in p else p["poss"].capitalize())
    s = s.replace("His/her", p["poss"].capitalize())

    # Convert explicit pronouns in the source bank to the selected gender.
    s = s.replace("He ", p["subj"] + " ")
    s = s.replace("She ", p["subj"] + " ")
    s = s.replace("he ", p["subj"].lower() + " ")
    s = s.replace("she ", p["subj"].lower() + " ")
    s = s.replace("him.", p["obj"] + ".").replace("her.", p["obj"] + ".")
    s = s.replace("him,", p["obj"] + ",").replace("her,", p["obj"] + ",")
    s = s.replace("his ", p["poss"] + " ").replace("her ", p["poss"] + " ")
    s = s.replace("His ", p["poss"].capitalize() + " ").replace("Her ", p["poss"].capitalize() + " ")

    # First sentence starts with the child's first name.
    if first_sentence:
        if s.startswith(("He ", "She ")):
            s = first_name + s[s.find(" "):]
        elif s.startswith(("His ", "Her ")):
            s = first_name + "'s " + s.split(" ", 1)[1]
        elif s.startswith(("he ", "she ")):
            s = first_name + s[s.find(" "):]
        elif s.startswith(("has ", "needs ", "can ", "is ", "shows ", "demonstrates ",
        ...
        if s.startswith(("has ", "needs ", "can ", "is ", "shows ", "demonstrates ",
                          "displays ", "recalls ", "requires ", "puts ", "makes ",
                          "uses ", "comprehends ", "shirks ", "tends ")):
            s = p["subj"] + " " + s
        elif s.startswith(("must ", "should ")):
            s = p["subj"] + " " + s
        elif s.startswith("His "):
            pass
        elif s.startswith("Her "):
            pass
        elif s.startswith(("his ", "her ")):
            s = p["poss"].capitalize() + s[3:]
        elif s.startswith("Solving ") and "would benefit" in s:
            s = s.replace("would benefit her.", f"would benefit {p['obj']}.")
            s = s.replace("would benefit him.", f"would benefit {p['obj']}.")
        elif s.startswith("A regular ") and ("will help him" in s or "will help her" in s):
            s = s.replace("will help him", f"will help {p['obj']}")
            s = s.replace("will help her", f"will help {p['obj']}")
        elif s.startswith("Analysis "):
            s = p["subj"] + "'s " + s[0].lower() + s[1:]
        elif s.startswith("Diagram "):
            s = p["subj"] + "'s " + s[0].lower() + s[1:]
        elif s.startswith("The concepts "):
            s = s
        elif s.startswith("Also "):
            s = s
        elif s.startswith("The laws "):
            s = s
    return s.strip()
