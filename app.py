...
st.sidebar.info("AI editing is optional. Click Adjust or Refine only when you want AI to change the draft.")
...
if st.session_state.stage == "upload":
    ...
    if st.session_state.generated:
        ...
        # The original generated draft stays visible and editable. AI never
        # replaces this box; AI output is shown separately below it.
        original_key = f"original_profile_editor_{i}_{subject}_{st.session_state.draft_version}"
        if original_key not in st.session_state:
            st.session_state[original_key] = st.session_state.profile_draft
        ...
        st.write(
            f"**Character count: {original_count}** (required: 366–390)  |  "
            f"**AI editing used:** {'Yes' if ai_used else 'No'}"
        )
        ...
        with c1:
            if st.button("✂️ Adjust to 366–390 characters", use_container_width=True):
                with st.spinner("AI is adjusting the profile to 366–390 characters…"):
                    ai_profile, ai_count, ai_status = ai_edit_profile(
                        original_draft, subject, name, gender, mode="adjust"
                    )
                if ai_status == "ai":
                    st.session_state.ai_profile = ai_profile
                    st.session_state.ai_profile_version += 1
                    st.session_state.generated = (
                        original_draft, original_count, "ok", selections, True
                    )
                    st.rerun()
        ...
        with c2:
            if st.button("✨ Refine with AI", use_container_width=True):
                # Refine the latest AI version when one exists; otherwise
                # refine the original teacher draft. The original box remains
                ...
                with st.spinner("AI is refining the profile…"):
                    ai_profile, ai_count, ai_status = ai_edit_profile(
                        refine_source, subject, name, gender, mode="refine"
                    )
                if ai_status == "ai":
                    st.session_state.ai_profile = ai_profile
                    st.session_state.ai_profile_version += 1
        ...
        # AI output is deliberately displayed in a separate box so the
        # teacher can compare the original and AI-edited versions.
        ...
        if ai_profile:
            ...
            edited_ai_profile = st.text_area(
                "AI version (editable)",
                height=190,
                key=ai_key
            )
            ...
            st.write(
                f"**Character count: {ai_count_current}** (required: 366–390)  |  "
                f"**AI editing used:** Yes"
            )
            ...
            elif ai_count_current > 0:
                st.warning("The AI version is below 366 characters. You can edit it manually or click Adjust again.")

        # Saving uses the AI version when one exists; otherwise it uses the
        # original editable profile. In either case the saved text must be
        ...
        if 366 <= final_count <= 390:
            if ai_profile:
                st.success("✓ A valid AI-edited profile is ready to save. You can still refine it further.")
            else:
            ...
            if st.button("Save & Next Child →", type="primary", use_container_width=True):
                st.session_state.profiles[i] = {
                    ...
                    "AI Edited": "Yes" if bool(ai_profile) else "No"
                }
                if i + 1 < len(df):
    ...
    if st.button("Start a new class / subject"):
        for key in ["students", "profiles", "generated", "profile_draft", "ai_profile"]:
            st.session_state[key] = None if key == "students" else ("" if key in ["profile_draft", "ai_profile"] else {})
        st.session_state.stage = "upload"
        st.session_state.student_index = 0
        st.rerun()
