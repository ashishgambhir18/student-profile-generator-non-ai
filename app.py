...
def sync_current_workspace():
    ...
    for cat, statements in bank.items():
        key = f"multi_{subject}_{i}_{cat}"
        selected_labels = st.session_state.get(key, [])
        labels = [prepare(statement, st.session_state.students.iloc[i]["Name"], st.session_state.students.iloc[i]["Gender"], first_sentence=False) for statement in statements]
        selected_set = set(selected_labels) if isinstance(selected_labels, list) else set()
        sel[cat] = [idx for idx, label in enumerate(labels) if label in selected_set]
    ws["selections"] = sel
...
if st.session_state.stage == "upload":
    ...
    # Compact category-based multi-selects. Each category is a dropdown, and
    # multiple statements can be selected within it. This keeps the page short
    # while retaining the full statement bank.
    selections = {}
    saved_sel = ws.get("selections", {})
    for cat, statements in bank.items():
        labels = [prepare(statement, name, gender, first_sentence=False) for statement in statements]
        saved_indices = saved_sel.get(cat, [])
        default_labels = [labels[idx] for idx in saved_indices if 0 <= idx < len(labels)]
        key = f"multi_{subject}_{i}_{cat}"
        selected_labels = st.multiselect(
            f"{cat} ({len(statements)} statements)",
            options=labels,
            default=default_labels,
            key=key,
            placeholder="Select one or more statements…",
            help="You can select multiple statements from this category."
        )
        selected_set = set(selected_labels)
        selections[cat] = [idx for idx, label in enumerate(labels) if label in selected_set]
