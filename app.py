...
def ai_edit_profile(profile, subject, name, gender, mode="adjust", max_attempts=3):
    ...
    for attempt in range(max_attempts):
        target = "Aim for 375–380 characters; keep a safety margin below 390." if attempt == 0 else "MUST be 366–390 characters inclusive; aim for about 375 characters and do not exceed 385 characters."
...
{target}"""
        try:
            ...
            if candidate and candidate.startswith(first_name):
                ...
                if n > 390:
                    task = "Shorten the latest draft while preserving its meaning, all important observations and correct grammar. Remove only redundancy or non-essential wording. Do not add new information."
                elif n < 366:
                    task = "Expand the latest draft carefully to 366–390 characters. Preserve every important teacher observation already present. Add only concise elaboration of the same observations or a closely related recommendation supported by the existing content. Do not invent any new facts, achievements, weaknesses, topics or behaviour. Keep the wording natural and professional."
